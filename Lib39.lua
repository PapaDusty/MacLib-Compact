...
function MacLib:Window(Settings)
	...
	minimizeIcon.Image = "rbxassetid://131130048783546"
	...
	minimizeIcon.Position = UDim2.new(1, -28, 0.5, 0)
	...
	minimizeIcon.MouseButton1Click:Connect(function()
    	...
    	windowState = base.Visible

    	WindowFunctions:Notify({
    	    Title = Settings.Title,
    	    Description = (base.Visible and "Maximized" or "Minimized") .. " the menu. Use RightControl to toggle it.",
    	    Lifetime = 3
    	})
	end)
			...
			function TabFunctions:Section(Settings)
				...
				function SectionFunctions:Slider(Settings, Flag)
    				...
    				if not showValueBox and headValue then
        				...
        				headValue.Position = UDim2.new(0.5, 0, 0, -4)
    				end
...
function TabFunctions:InsertThemesSection(Side)
    ...
    -- Create dropdown first so it appears above
    local themeDropdown = themesSection:Dropdown({
        ...
        Callback = function(selected)
            ...
            -- Label will be created after, but we can reference it via upvalue
            -- But label doesn't exist yet! So we need to define callback after label.
        end
    })

    -- Create label (will appear below dropdown because dropdown was created first)
    local currentThemeLabel = themesSection:Label({ Text = "Current theme: Dark" })

    -- Now set the dropdown's callback (label exists now)
    ...
    -- Optionally set LayoutOrder for clarity (dropdown = 1, label = 2)
    themeDropdown.LayoutOrder = 1
    currentThemeLabel.LayoutOrder = 2
end
